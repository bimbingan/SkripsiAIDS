...
class agenda extends ApplicationBase{
    ...
    function index() {
        ...
        $this->smarty->assign("template_content", "agenda/list.html");
         $this->smarty->load_javascript("resource/js/ckeditor/ckeditor.js");


        /* PART 3 : load data from database */
        $result = $this->m_preference->get_preference_by_id("89");
        ...
        function process_edit(){
                ...
                $this->tnotification->set_rules('agenda_id', 'ID', 'trim|required');
                $this->tnotification->set_rules('agenda', 'Agenda KPA', 'trim|max_length[10000000000000000000]');

                if($this->tnotification->run() !== FALSE){
                    $params = array(
                        'pref_value' => $this->input->post('agenda')
                    );

                    $where = array(
                        'pref_id' => $this->input->post('agenda_id')
                    );
