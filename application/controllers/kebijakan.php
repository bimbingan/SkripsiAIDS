...
class kebijakan extends ApplicationBase{
    ...
    function index() {
        ...
        $this->smarty->assign("template_content", "kebijakan/list.html");
         $this->smarty->load_javascript("resource/js/ckeditor/ckeditor.js");


        /* PART 3 : load data from database */
        $result = $this->m_preference->get_preference_by_id("88");
        ...
        function process_edit(){
                ...
                $this->tnotification->set_rules('kebijakan_id', 'ID', 'trim|required');
                $this->tnotification->set_rules('kebijakan', 'Kebijakan KPA', 'trim|max_length[10000000000000000000]');

                if($this->tnotification->run() !== FALSE){
                    $params = array(
                        'pref_value' => $this->input->post('kebijakan')
                    );

                    $where = array(
                        'pref_id' => $this->input->post('kebijakan_id')
                    );
